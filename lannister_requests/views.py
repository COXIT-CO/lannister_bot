...
class BonusRequestViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.query_params.get("reviewer") is not None and self.request.query_params.get(
            "reviewable"
        ) is not None:
            print('here')
            """
    ...
    def create(self, request):
        ...
        is_reviewer = any(
            item.get("name") == "Administrator" for item in creator.roles.all()
        )
        if is_reviewer:
            return Response(
                data={
    ...
    def partial_update(self, request, pk=None):
        ...
        if request.data.get("bonus_type") and request.data.get('description'):
            bonus_rq = provided_bonus_request_to_patch.__class__.objects.update(
